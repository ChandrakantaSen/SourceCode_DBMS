select MiscDb.dbo.pm_depot_master.regn,
	   ...
	   No_of_Dealers=
		   (case 
			When MiscDb.dbo.dn_dealer_list.dealer_category='GC' 
				then COUNT(MiscDb.dbo.dn_dealer_list.dealer_category)
			When MiscDb.dbo.dn_dealer_list.dealer_category='SC' 
				then COUNT(MiscDb.dbo.dn_dealer_list.dealer_category)
			When MiscDb.dbo.dn_dealer_list.dealer_category='STA' 
				then COUNT(MiscDb.dbo.dn_dealer_list.dealer_category)
			When MiscDb.dbo.dn_dealer_list.dealer_category='OTH' 
				then COUNT(MiscDb.dbo.dn_dealer_list.dealer_category)
			Else Null
			End)	 	   
