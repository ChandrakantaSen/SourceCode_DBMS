SELECT IDRV.REGN                                                  REGN
      ...
      ,SUM(CASE        
               WHEN TO_NUMBER(TO_CHAR(IDRV.INV_DATE,'YYYYMM'))>=201204 AND TO_NUMBER(TO_CHAR(IDRV.INV_DATE,'YYYYMM'))<=201303  THEN IDRV.FNL_VOL                
               ELSE 0
           END)  LY,
           SUM(CASE        
               WHEN TO_NUMBER(TO_CHAR(IDRV.INV_DATE,'YYYYMMDD'))<=20140431 AND TO_NUMBER(TO_CHAR(IDRV.INV_DATE,'YYYYMMDD'))>=20140401  THEN IDRV.FNL_VOL                
               ELSE 0
           END)  TY                                                 
  FROM
       -- ******************************* -- 
          ...
          (
            SELECT MSIB.INVENTORY_ITEM_ID
             ...
             WHERE MSIB.ORGANIZATION_ID = 102 
               ...
               AND SUBSTR(MSIB.SEGMENT1,1,6) IN ('F00399','F00B02')
          ) MSIB,
     ...
     WHERE RCTA.CUSTOMER_TRX_ID            = RCTLA.CUSTOMER_TRX_ID   
       ...
       AND TO_NUMBER(TO_CHAR(RCTA.TRX_DATE,'YYYYMM'))>=201204
      )  IDRV,
...
GROUP BY IDRV.REGN                                                                                         
        ,NVL(TERR_MASTER.TERR_CODE,IDRV.TERR_CODE) 
        ,IDRV.DEPO              
