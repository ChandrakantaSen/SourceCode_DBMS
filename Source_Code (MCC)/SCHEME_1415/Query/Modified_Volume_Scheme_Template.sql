...
,CHLD_CODE   BILL_TO
,SUM(CASE        
	WHEN TO_NUMBER(TO_CHAR(INV_DATE,'YYYYMMDD')) <= 20140430                             --DATED 30.04.2014 
	AND TO_NUMBER(TO_CHAR(INV_DATE,'YYYYMMDD')) >= 20140401  THEN VOLUME                --DATED 01.04.2014 
		ELSE 0
	END)  TY_VOL  
	,SUM(CASE        
		WHEN TO_NUMBER(TO_CHAR(INV_DATE,'YYYYMMDD')) <= 20140427                            --DATED 30.04.2014 
		AND TO_NUMBER(TO_CHAR(INV_DATE,'YYYYMMDD')) >= 20140401  THEN VOLUME                --DATED 01.04.2014 
			ELSE 0
		END)  TY_VOL_27   
		FROM
